...
class AuthErrorHandlerMiddleware:
    ...
    async def _send_error_response(self, response: JSONResponse, send: Callable) -> None:
...
def add_error_handlers(app) -> None:
    ...
    async def token_missing_exception_handler(request: Request, exc: TokenMissingError):
        return await auth_exception_handler(request, exc)
    ...
    async def travel_companion_exception_handler(request: Request, exc: TravelCompanionError):
        return await auth_exception_handler(request, exc)