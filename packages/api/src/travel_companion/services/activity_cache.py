...
class ActivityCacheManager:
    ...
    async def cache_search_results(
        ...
        try:
            ...
            response_dict["cache_expires_at"] = (
    ...
    async def warm_popular_destinations(self, destinations: list[dict[str, Any]]) -> int:
        ...
        try:
            ...
            for i in range(0, len(destinations), self.CACHE_WARMING_BATCH_SIZE):
                ...
                warming_tasks = [
                    self._warm_destination_cache(dest) for dest in batch
                ]
                ...
                warmed_count += sum(1 for result in results if result and not isinstance(result, Exception))
    ...
    async def _generate_search_cache_key(self, request: ActivitySearchRequest) -> str:
        ...
        key_parts = [
            self.SEARCH_PREFIX,
            request.location.lower().replace(" ", "_")[:20],
            request.category.value if request.category else "any",
    ...
    async def _calculate_dynamic_ttl(
        ...
        # Longer TTL for popular destinations
        if await self._is_popular_destination(request.location):
            base_ttl = self.POPULAR_DESTINATION_TTL
        
        # Shorter TTL for searches with few results (likely to change)
    ...
    async def _cache_location_data(
        ...
        try:
            ...
            location_data = {
                ...
                "avg_price": sum(a.price for a in response.activities) / len(response.activities) if response.activities else 0,
                "categories": list(set(a.category for a in response.activities)),
