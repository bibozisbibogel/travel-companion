...
class ActivityRepository:
    ...
    async def insert_activity_options(
        self, activities: list[ActivityOption], trip_id: UUID
    ) -> list[UUID]:
        ...
        try:
            ...
            for activity in activities:
                activity_data = {
                    ...
                    if hasattr(activity.category, "value")
                    else str(activity.category),
                    "location": {
    ...
    async def update_activity_rating(
        self, activity_id: UUID, rating: float, review_count: int
    ) -> bool:
