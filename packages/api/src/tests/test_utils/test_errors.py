...
class TestAuthenticationError:
    ...
    def test_custom_authentication_error(self):
        ...
        error = AuthenticationError(
            message="Custom auth error", error_code=AuthErrorCode.TOKEN_EXPIRED, details=details
        )
...
class TestUserNotFoundError:
    ...
    def test_user_not_found_error_with_user_id(self):
        ...
        user_id = str(uuid4())
        error = UserNotFoundError(message="User does not exist", user_id=user_id)
...
class TestUserAlreadyExistsError:
    ...
    def test_user_already_exists_error_with_email(self):
        ...
        error = UserAlreadyExistsError(message="Email already registered", email=email)
...
class TestTokenErrors:
    ...
    def test_token_expired_error_with_details(self):
        ...
        error = TokenExpiredError(message="JWT token expired", details=details)
    ...
    def test_invalid_token_error_with_details(self):
        ...
        error = InvalidTokenError(message="JWT signature invalid", details=details)
...
class TestExternalAPIError:
    ...
    def test_basic_external_api_error(self):
        ...
        error = ExternalAPIError(message="Service unavailable", service="payment_gateway")

        assert error.message == "Service unavailable"
        assert error.error_code is None
        assert error.details == {"service": "payment_gateway", "status_code": None}

    def test_external_api_error_with_status_code(self):
        """Test external API error with HTTP status code."""
        error = ExternalAPIError(
            message="Bad request", service="booking_api", status_code=400, error_code="EXT001"
        )

        assert error.message == "Bad request"
        assert error.error_code == "EXT001"
        assert error.details == {"service": "booking_api", "status_code": 400}
...
class TestErrorResponseHelpers:
    ...
    def test_create_error_response_basic(self):
        ...
        expected = {"error": True, "message": "Test error", "data": None, "status_code": 400}
    ...
    def test_create_error_response_with_code_and_details(self):
        ...
        response = create_error_response(
            message="Validation failed", error_code="VAL001", details=details, status_code=422
        )
    ...
    def test_create_auth_error_response_custom(self):
        ...
        details = {"reason": "token_expired"}
        response = create_auth_error_response(
            message="Token has expired", error_code=AuthErrorCode.TOKEN_EXPIRED, details=details
        )
