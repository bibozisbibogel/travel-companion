...
class TestHotelAgentFallback:
    ...
    async def test_successful_booking_api_no_fallback(self, hotel_agent, sample_search_request, booking_api_response):
        """Test successful hotel search using Booking.com API without fallback."""
        with patch.object(hotel_agent._booking_client, 'search_hotels', return_value=booking_api_response):
    ...
    async def test_fallback_to_expedia_when_booking_fails(
        ...
        with patch.object(
            ...
            'search_hotels', 
            return_value=expedia_api_response
        ):
            response = await hotel_agent._search_hotels(sample_search_request)
    ...
    async def test_fallback_to_airbnb_when_booking_and_expedia_fail(
        ...
        with patch.object(
            ...
            'search_listings', 
            return_value=airbnb_api_response
        ):
            response = await hotel_agent._search_hotels(sample_search_request)
    ...
    async def test_performance_tracking_across_fallbacks(
        ...
        with patch.object(
            ...
            'search_hotels', 
            return_value=expedia_api_response
        ), patch('time.time', side_effect=[1000.0, 1000.05, 1000.1, 1000.15]):  # Mock time values for multiple calls
            response = await hotel_agent._search_hotels(sample_search_request)
    ...
    async def test_partial_failures_with_data_conversion_errors(self, hotel_agent, sample_search_request):
        """Test handling of partial failures during data conversion."""
        # Mock response with some valid and some invalid data
        from travel_companion.services.external_apis.booking import BookingHotelResult, BookingApiResponse
        ...
        with patch.object(hotel_agent._booking_client, 'search_hotels', return_value=booking_response):
            response = await hotel_agent._search_hotels(sample_search_request)
    ...
    async def test_airbnb_max_price_filter_integration(self, hotel_agent, airbnb_api_response):
        ...
        with patch.object(
            ...
            call_args = mock_airbnb.call_args[0][0]  # First positional argument (AirbnbSearchParams)
    ...
    async def test_concurrent_api_calls_not_implemented(self, hotel_agent, sample_search_request):
        ...
        with patch.object(hotel_agent._booking_client, 'search_hotels', side_effect=mock_booking_search), \
             patch.object(hotel_agent._expedia_client, 'search_hotels', side_effect=mock_expedia_search), \
             patch.object(hotel_agent._airbnb_client, 'search_listings', side_effect=mock_airbnb_search):
            
            response = await hotel_agent._search_hotels(sample_search_request)
            
            # Verify sequential execution order
            assert call_order == ["booking", "expedia", "airbnb"]
            assert response.search_metadata["apis_attempted"] == ["booking.com", "expedia", "airbnb"]