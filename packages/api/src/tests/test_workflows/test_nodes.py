...
from travel_companion.models.trip import TripPlanRequest, TripRequirements, TripDestination, TravelClass
...
class TestTripPlanningNodes:
    ...
    async def test_execute_hotel_agent_success(self, mock_logger, mock_hotel_agent_class, sample_workflow_state):
        ...
        mock_hotel = HotelOption(
            ...
            star_rating=4,
            price_per_night=Price(amount=Decimal("150.00"), currency="USD"),
            amenities=["wifi", "pool"],
            availability=True,
    ...
    async def test_execute_activity_agent_with_weather_dependency(self, mock_logger, mock_activity_agent_class, sample_workflow_state):
        ...
        mock_activity = ActivityOption(
            ...
            estimated_cost=Price(amount=Decimal("25.00"), currency="USD"),
            location="Paris",
        ...
        # Verify weather conditions were passed to activity agent
        call_args = mock_activity_agent.process.call_args[0][0]
        assert "weather_conditions" in call_args
        assert call_args["weather_conditions"] == ["sunny", "partly_cloudy", "rainy"]
    ...
    async def test_dependency_management_weather_to_activities(self, sample_workflow_state):
        ...
        with patch("travel_companion.workflows.nodes.ActivityAgent") as mock_activity_agent_class:
            ...
            # Verify weather conditions were passed to activity request
            call_args = mock_activity_agent.process.call_args[0][0]
            assert "weather_conditions" in call_args
            assert call_args["weather_conditions"] == ["sunny", "rainy"]
